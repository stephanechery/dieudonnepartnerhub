...
export default function ModuleCard({ module, onOpen, darkMode = false, translateText = (value) => value }) {
  ...
  return (
    <article className={`relative overflow-hidden rounded-[1.5rem] border p-4 sm:p-5 transition duration-200 ${
      module.unlocked
        ? darkMode
      ...
      <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <h3 className={`text-lg font-black leading-tight ${darkMode ? "text-slate-100" : "text-slate-900"}`}>{tx(module.title)}</h3>
      ...
      <div className="space-y-2">
        ...
        <div className={`flex flex-col gap-1 text-sm sm:flex-row sm:items-center sm:justify-between ${darkMode ? "text-slate-400" : "text-slate-600"}`}>
